...
class ListsSuite extends FunSuite {
  ...
  test("sum of negative numbers") {
    assert(sum(List(-3, -5, -10)) === -18)
  }

  test("sum of negative and positive numbers") {
    assert(sum(List(-4, 8, 1, 0)) === 5)
  }

  test("sum of just zeros") {
  ...
  test("max of a same numbers") {
    assert(max(List(3, 3, 0)) === 3)
  }
  ...
  test("max of a negative and positive numbers") {
    assert(max(List(-3, 5, 0)) === 5)
  }

  test("max of just zeroes") {
